...
def get_computer(computer_id: int,db: Session = Depends(get_db)):
    commands = crud.get_commands_by(db=db,target_id=computer_id)
    computer = crud.get_computer(db, computer_id=computer_id)
    return {'computer':computer,'commands':commands}

@app.post("/computer/{computer_id}/ping")
def set_ping(computer_id: int, ping: schemas.PingBase, db: Session = Depends(get_db)):
    computer = crud.set_ping(db,computer_id, ping)
    return computer
